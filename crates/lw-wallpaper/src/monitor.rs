...
pub fn get_monitors(wallpaper: &IDesktopWallpaper) -> Result<Vec<MonitorInfo>, LwError> {
    ...
    unsafe {
        ...
        for i in 0..count {
            let path = wallpaper.GetMonitorDevicePathAt(i)
                .map_err(|e| LwError::Wallpaper(format!("Failed to get monitor path at index {i}: {e}")))?;

            let monitor_id_pcwstr = PCWSTR(path.0);
            let bounds = wallpaper.GetMonitorRECT(monitor_id_pcwstr)
                .map_err(|e| LwError::Wallpaper(format!("Failed to get monitor bounds for index {i}: {e}")))?;
