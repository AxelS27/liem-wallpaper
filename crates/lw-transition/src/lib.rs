...
use lw_renderer::{D3D11Context, CompositionContext, load_texture_from_file};
use std::path::{Path, PathBuf};
use std::sync::Arc;
use windows::Win32::Graphics::Direct3D11::{
    ID3D11VertexShader, D3D11_BUFFER_DESC, D3D11_BIND_CONSTANT_BUFFER,
    D3D11_USAGE_DYNAMIC, D3D11_CPU_ACCESS_WRITE, ID3D11RenderTargetView,
    ID3D11SamplerState, D3D11_SAMPLER_DESC, D3D11_FILTER_MIN_MAG_MIP_LINEAR,
    D3D11_TEXTURE_ADDRESS_CLAMP, D3D11_MAP_WRITE_DISCARD, D3D11_VIEWPORT,
};
use windows::Win32::Graphics::Dxgi::{
    IDXGIFactory2, IDXGISwapChain1, DXGI_SWAP_CHAIN_DESC1, DXGI_USAGE_RENDER_TARGET_OUTPUT,
...
use windows::Win32::Foundation::{RECT, FALSE};
...
impl TransitionEngine {
    pub fn new(
        ...
        let dxgi_device: windows::Win32::Graphics::Dxgi::IDXGIDevice = d3d_device.cast()
            .map_err(|e| LwError::Renderer(format!("Failed to cast D3D11 device to DXGI device: {e}")))?;
        ...
        unsafe {
            d3d_device.CreateSamplerState(&sampler_desc, Some(&mut sampler))
                .map_err(|e| LwError::Renderer(format!("Failed to create SamplerState: {e}")))?;
        }
        let sampler = sampler.ok_or_else(|| LwError::Renderer("SamplerState is null".to_string()))?;
        ...
        let vertex_shader = vertex_shader.ok_or_else(|| LwError::Renderer("VertexShader is null".to_string()))?;
        ...
        for bounds in monitors_bounds {
            ...
            unsafe {
                ...
                visual.SetOffsetX2(bounds.left as f32)
                    .map_err(|e| LwError::Renderer(format!("Failed to set visual offset X: {e}")))?;
                #[allow(clippy::cast_precision_loss)]
                visual.SetOffsetY2(bounds.top as f32)
                    .map_err(|e| LwError::Renderer(format!("Failed to set visual offset Y: {e}")))?;
            }
            ...
            let swapchain = unsafe {
                dxgi_factory.CreateSwapChainForComposition(d3d_device, &swapchain_desc, None)
                    .map_err(|e| LwError::Renderer(format!("Failed to create swapchain for composition: {e}")))?
            };
            ...
            unsafe {
                ...
                comp_context.root_visual().AddVisual(&visual, true, None)
                    .map_err(|e| LwError::Renderer(format!("Failed to add visual to root tree: {e}")))?;
            }

            // Get Render Target View (RTV)
            let back_buffer: windows::Win32::Graphics::Direct3D11::ID3D11Texture2D = unsafe { swapchain.GetBuffer(0) }
            ...
            unsafe {
                d3d_device.CreateRenderTargetView(&back_buffer, None, Some(&mut rtv))
                    .map_err(|e| LwError::Renderer(format!("Failed to create RenderTargetView: {e}")))?;
            }
            let rtv = rtv.ok_or_else(|| LwError::Renderer("RenderTargetView is null".to_string()))?;

            monitors.push(MonitorRenderContext {
                swapchain,
                rtv,
                visual,
                bounds: *bounds,
...
impl TransitionRenderer for TransitionEngine {
    fn render_transition(
        ...
        // 1. Load textures
        let (_tex_from, srv_from, _width_from, _height_from) = load_texture_from_file(d3d_device, from_image)?;
        let (_tex_to, srv_to, _width_to, _height_to) = load_texture_from_file(d3d_device, to_image)?;

        // 2. Load and Compile Pixel Shader
        let shader_path = self.shader_dir.join(format!("{effect_type}.hlsl"));
        let ps_src = std::fs::read_to_string(&shader_path).unwrap_or_else(|_| DEFAULT_PS_SRC.to_string());
        ...
        let pixel_shader = pixel_shader.ok_or_else(|| LwError::Renderer("PixelShader is null".to_string()))?;
        ...
        unsafe {
            d3d_device.CreateBuffer(&cb_desc, None, Some(&mut constant_buffer))
                .map_err(|e| LwError::Renderer(format!("Failed to create constant buffer: {e}")))?;
        }
        let constant_buffer = constant_buffer.ok_or_else(|| LwError::Renderer("Constant buffer is null".to_string()))?;
        ...
        loop {
            ...
            unsafe {
                ...
                d3d_device_context.Map(&constant_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, Some(&mut mapped_resource))
                    .map_err(|e| LwError::Renderer(format!("Failed to map constant buffer: {e}")))?;
            ...
            for monitor in &self.monitors {
                ...
                unsafe {
                    ...
                    d3d_device_context.IASetPrimitiveTopology(windows::Win32::Graphics::Direct3D::D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
                    d3d_device_context.VSSetShader(&self.vertex_shader, None);
                    d3d_device_context.PSSetShader(&pixel_shader, None);
                    
                    d3d_device_context.PSSetShaderResources(0, Some(&[Some(srv_from.clone()), Some(srv_to.clone())]));
        ...
        unsafe {
            let mut mapped_resource = windows::Win32::Graphics::Direct3D11::D3D11_MAPPED_SUBRESOURCE::default();
            let _ = d3d_device_context.Map(&constant_buffer, 0, D3D11_MAP_WRITE_DISCARD, 0, Some(&mut mapped_resource));
