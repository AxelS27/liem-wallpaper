...
float4 main(PS_INPUT input) : SV_Target {
    ...
    // Zoom effect: incoming texture starts small (0.7x) and expands to 1.0x
    float scale = lerp(0.7, 1.0, progress);
    
    float2 uvFrom = get_fill_uv(input.tex, textureFrom, aspect);
    float2 baseTo = get_fill_uv(input.tex, textureTo, aspect);
    float2 texTo = (baseTo - 0.5) / scale + 0.5;
